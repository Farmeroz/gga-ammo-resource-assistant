...
function resolveLoadoutRecords(actor, loadout, { needsReserve = false } = {}) {
  ...
  if (!attackResult.record) {
    throw new Error(attackResult.ambiguous
  ...
  if (!ammoResult.record) {
    throw new Error(ammoResult.ambiguous
  ...
  if (needsReserve && loadout.reserve && !reserveResult.record) {
    throw new Error(reserveResult.ambiguous
...
async function repairReferences(actor, loadout, resolved) {
  if (!resolved.attackResult.relinked && !resolved.ammoResult.relinked && !resolved.reserveResult.relinked) return loadout;
  const repaired = {
    ...loadout,
  ...
  try {
    ...
    console.warn(`${MODULE_ID} | Loadout ran successfully but its repaired references could not be saved.`, error);
    return repaired;
  }
}
...
async function chooseInsufficientAction({ requestedShots, affordableShots, available, allowOverride }) {
  const buttons = [];
...
function visibilityData(visibility, inherited = null) {
  ...
  const resolved = visibility === VISIBILITY.INHERIT
  ...
  if (resolved === VISIBILITY.GM) {
    const recipients = new Set([game.user.id, ...ChatMessage.getWhisperRecipients("GM").map(user => user.id)]);
...
async function postReceipt({ actor, title, summary, details = [], changes, warning = "", visibility, inheritedVisibility }) {
  if (!game.settings.get(MODULE_ID, SETTINGS.CHAT_RECEIPTS)) return null;
  const detailHtml = details.filter(Boolean).map(detail => `<span>${escapeHtml(detail)}</span>`).join("");
  const content = `
...
async function postReceiptSafely(options) {
  try {
    ...
    console.error(`${MODULE_ID} | The resource changed, but its chat receipt could not be posted.`, error);
    ui.notifications.warn("The resource was updated, but its chat receipt could not be posted. Undo is unavailable for this change.");
    return null;
  }
}
...
export async function executeFire({ actor, loadout, shots = null, promptIfConfigured = true } = {}) {
  try {
    ...
    const duplicates = resolved.attacks.filter(candidate => attackLabel(candidate) === attackLabel(attack));
    if (duplicates.length > 1) {
      throw new Error(`More than one ranged attack is named ${attackLabel(attack)}. Give the usages distinct names or modes in GGA.`);
    }

    const maximumRof = parseRateOfFire(attack.rof);
    const available = Math.max(trackerValue(actor, ammo.path) - minimum, 0);
    const maxAffordable = maximumAffordableShots(available, loadout.unitsPerShot, loadout.flatCost, maximumRof);

    let chosenShots = shots == null ? wholeNumber(loadout.shots, maximumRof, 1) : wholeNumber(shots, 1, 1);
    ...
    if (shots == null && promptIfConfigured && loadout.hotbarShotMode === "ask" && maximumRof > 1) {
      ...
      chosenShots = await chooseShots({ maximum: maximumRof, available: maxAffordable, initial: chosenShots });
    ...
    if (available < cost) {
      const decision = await chooseInsufficientAction({
        ...
        available,
        allowOverride: Boolean(game.user.isGM && game.settings.get(MODULE_ID, SETTINGS.GM_OVERRIDES))
      });
    ...
    if (!roll.rolled) {
      ui.notifications.warn("The attack was cancelled or could not be rolled. No ammunition was spent.");
      return { ok: false, reason: "not-rolled" };
    }
    if (roll.promptExpected && !roll.promptSeen) {
      await postCompatibilityWarning(actor, loadout);
      ui.notifications.error("The attack rolled, but its shot count was not captured. Ammunition was not changed.");
    ...
    if (threshold > 0 && remaining <= threshold) warnings.push(`${ammo.name} is low: ${remaining} available.`);
...
export async function executeReload({ actor, loadout, amount = null } = {}) {
  try {
    ...
    if (ammo.max <= 0) throw new Error(`${ammo.name} needs a maximum value before it can be reloaded.`);
    ...
    if (reload.transferred <= 0) {
      ui.notifications.info(reload.availableSpace <= 0 ? `${ammo.name} is already full.` : "No reserve ammunition is available.");
    ...
    await postReceiptSafely({
      ...
      details: [
        ...
        reserve ? `${reserve.name}: ${reserveBefore} → ${reload.reserveAfter}` : "Untracked replenishment",
        attack.shots ? `GGA Shots: ${attack.shots}` : ""
      ],
      changes,
      warning: reload.shortfall > 0 ? `Reserve ammunition was ${reload.shortfall} short of the requested reload.` : "",
      visibility: loadout.visibility
    });
...
export async function executeAdjustment({ actor, trackerReference, operation, amount, visibility = VISIBILITY.INHERIT } = {}) {
  try {
    assertOwnership(actor);
...
export async function undoReceipt(message) {
  ...
  if (!actor || (!actor.isOwner && !game.user.isGM)) {
    ui.notifications.warn("The actor is unavailable or you no longer have permission to change it.");
    return false;
  }
  if (!canUndo(receipt.changes, path => foundry.utils.getProperty(actor, path))) {
    ui.notifications.warn("The resource has changed since this receipt was created, so it was not overwritten.");
    return false;
  }
