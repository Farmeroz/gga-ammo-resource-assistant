...
export function maximumAffordableShots(available, unitsPerShot = 1, flatCost = 0, maximum = Infinity) {
...
export function calculateReload({ current, capacity, reserve, requested, mode = "to-full", hasReserve = true }) {
  ...
  const wanted = mode === "to-full"
    ? availableSpace
    : Math.min(wholeNumber(requested, 0, 0), availableSpace);
...
export function calculateAdjustment(current, operation, amount, { minimum = null, maximum = null } = {}) {
...
export function resolveReference(records, reference, kind = "tracker") {
  if (!reference || !Array.isArray(records)) return { record: null, relinked: false, ambiguous: false };

  const pathMatch = records.find(record => record.path === reference.path);
  if (pathMatch) {
    const identityMatches = kind === "attack"
      ? normalise(pathMatch.name) === normalise(reference.name) && normalise(pathMatch.mode) === normalise(reference.mode)
  ...
  if (kind === "attack" && reference.uuid) {
    ...
    if (uuidMatches.length === 1) return { record: uuidMatches[0], relinked: true, ambiguous: false };
...
export function makeLoadout(input = {}, randomId = () => crypto.randomUUID()) {
  const loadout = {
    ...
    visibility: Object.values(VISIBILITY).includes(input.visibility) ? input.visibility : VISIBILITY.INHERIT
  };
  return loadout;
}
...
export function validateLoadout(loadout, { maximumRof = Infinity } = {}) {
  ...
  if (wholeNumber(loadout?.shots, 0) > maximumRof) errors.push(`Shots cannot exceed RoF ${maximumRof}.`);
...
export function canUndo(changes, getCurrentValue) {
  return Array.isArray(changes) && changes.length > 0 && changes.every(change => (
...
export function parseImportPayload(payload, randomId = () => crypto.randomUUID()) {
  if (!payload || payload.type !== "gga-ammo-resource-assistant-loadouts" || !Array.isArray(payload.loadouts)) {
