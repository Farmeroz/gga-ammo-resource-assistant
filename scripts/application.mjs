...
function suggestedCapacity(attack) {
  const match = String(attack?.shots ?? '')
    .trim()
    .match(/^\s*(\d+)/);
...
export class AmmoAssistantApp extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  async _prepareContext() {
    ...
    const ammo =
      trackers.find((record) => record.path === this.draft.ammoPath) ??
      suggestTracker(attack, trackers);
    ...
    const affordableShots = ammo
      ? Math.max(
          ...
          Math.min(
            maximumRof,
            Math.floor(
              (ammo.value - minimum - this.draft.flatCost) / Math.max(this.draft.unitsPerShot, 1),
            ),
          ),
        )
      : 0;
    const ammoPercent =
      ammo?.max > 0 ? Math.max(0, Math.min(100, Math.round((ammo.value / ammo.max) * 100))) : 0;
    const canModifyActor = Boolean(actor.isOwner || game.user.isGM);
    const gmCanOverride = Boolean(
    ...
    return {
      ...
      allTrackers: optionise(
        trackerOptionLabels(allTrackers),
        this.draft.adjustTrackerPath,
        (record) => record.label,
      ),
      attack,
      ammo,
    ...
    return [
      {
        ...
        label: 'Fire',
        icon: 'fa-solid fa-bullseye',
        active: this.mode === ACTIONS.FIRE,
      },
      {
        id: ACTIONS.RELOAD,
        label: 'Reload',
        icon: 'fa-solid fa-arrows-rotate',
        active: this.mode === ACTIONS.RELOAD,
      },
      {
        id: ACTIONS.ADJUST,
        label: 'Adjust',
        icon: 'fa-solid fa-sliders',
        active: this.mode === ACTIONS.ADJUST,
      },
      {
        id: ACTIONS.LOADOUTS,
        label: 'Loadouts',
        icon: 'fa-solid fa-list',
        active: this.mode === ACTIONS.LOADOUTS,
      },
    ];
  }
  ...
  async _saveCurrent() {
    ...
    this.draft = this._draftFromLoadout(
      saved,
  ...
  async _switchActor(actorUuid) {
    ...
    if (!actor || (!actor.isOwner && !game.user.isGM))
      throw new Error('That actor is unavailable or not owned by you.');
    this.actor = actor;
  ...
  async _runSetupWizard() {
    const attacks = collectRangedAttacks(this.actor);
    if (!attacks.length)
      throw new Error('Add a ranged attack to the actor before setting up ammunition.');
    const initialAttack =
      attacks.find((record) => record.path === this.draft?.attackPath) ?? attacks[0];
      ...
      render: (_event, dialog) => {
        ...
        const syncReserve = () =>
          reserveFields.forEach((field) => {
            if (field) field.disabled = !reserveToggle?.checked;
          });
  ...
  async _handleAction(action, target) {
    try {
      ...
      if (action === 'fire') {
        ...
        const result = await executeFire({
          actor: this.actor,
          loadout,
          shots: this.draft.shots,
      ...
      if (action === 'adjust') {
        this._readForm();
        const tracker = collectResourceTrackers(this.actor, { includeDamage: true }).find(
          (record) => record.path === this.draft.adjustTrackerPath,
        );
      ...
      if (action === 'load-loadout') {
        const loadout = getLoadouts(this.actor).find(
          (item) => item.id === target.dataset.loadoutId,
        );
        if (!loadout) throw new Error('The saved loadout was not found.');
      ...
      if (action === 'duplicate-loadout') {
        ...
        if (!loadout) throw new Error('The saved loadout was not found.');
        const duplicate = makeLoadout({ ...loadout, id: '', name: `${loadout.name} Copy` }, () =>
          foundry.utils.randomID(),
        );
      ...
      if (action === 'delete-loadout') {
        ...
        const removedMacros =
          decision === 'delete-all' ? await deleteLoadoutMacros(this.actor.uuid, loadoutId) : 0;
      ...
      if (action === 'export-loadouts') {
        ...
        saveDataToFile(
          ...
          'application/json',
          `${slug(this.actor.name)}-ammo-loadouts.json`,
        );
        return;
      }
  ...
  async _importFile(file) {
    try {
      ...
      ui.notifications.info(
        `${imported.length} loadout${imported.length === 1 ? '' : 's'} imported. Relink any unmatched trackers before use.`,
      );
      this.render({ force: true });
    } catch (error) {
      log.error(error);
  ...
  _onRender(context, options) {
    ...
    this.element
      .querySelector('select[name="selectedLoadoutId"]')
