...
export function collectResourceTrackers(actor, { includeDamage = false } = {}) {
  ...
  return Object.entries(root)
    .filter(
      ([, tracker]) => tracker && typeof tracker === 'object' && String(tracker.name ?? '').trim(),
    )
    .map(([key, tracker]) => ({
      key,
...
function isRangedAttack(value) {
  ...
  return ['rof', 'shots', 'acc', 'rcl', 'range', 'damage', 'level', 'import'].some(
    (key) => key in value,
  );
}
...
export async function createResourceTrackers(actor, definitions) {
  ...
  const names = new Set(
    Object.values(existingRoot)
      .map((tracker) => normalise(tracker?.name))
      .filter(Boolean),
  );
  ...
  for (const definition of requested) {
    ...
    if (names.has(key) || createdNames.has(key))
      throw new Error(`A resource tracker named ${name} already exists.`);
    ...
    if (!Number.isFinite(maximum) || maximum < 1)
      throw new Error(`${name} needs a maximum of at least 1.`);
  ...
  let nextIndex =
    Math.max(
      -1,
      ...Array.from(usedKeys).map((key) => (/^\d+$/.test(key) ? Number.parseInt(key, 10) : -1)),
    ) + 1;
  const updates = {};
  const pending = [];
  for (const definition of requested) {
...
function tokenDocumentForActor(actor) {
  ...
  const token =
    canvas?.tokens?.placeables?.find((candidate) => candidate.actor === actor) ??
...
class ShotPromptBridge {
  ...
  async run({ actor, attackName, shots, expectsPrompt }, callback) {
    if (this.active)
      throw new Error('Another ammunition-assisted attack is already in progress on this client.');
...
export async function rollRangedAttack(actor, attack, shots) {
  ...
  if (!globalThis.GURPS?.executeOTF)
    throw new Error('GURPS Game Aid attack execution was not found.');
  if (!shotPromptBridge.installed)
    throw new Error('The GGA shot-selection compatibility layer is not ready.');
  ...
  const transaction = await shotPromptBridge.run(
    ...
    async () => {
      ...
      try {
        ...
        return await GURPS.executeOTF(
          rangedAttackOtf(attack),
          false,
          {
            shiftKey: false,
            ctrlKey: false,
...
export function suggestTracker(attack, trackers) {
  ...
  if (ammoText) {
    ...
    const related = trackers.filter(
      (tracker) =>
        ammoText.includes(normalise(tracker.name)) || normalise(tracker.name).includes(ammoText),
    );
    if (related.length === 1) return related[0];
  }

  const words = normalise(attack?.name)
    .split(/[^a-z0-9]+/)
    .filter((word) => word.length >= 3);
  const scored = trackers
    .map((tracker) => ({
      tracker,
      score: words.reduce(
        (score, word) => score + (normalise(tracker.name).includes(word) ? 1 : 0),
        0,
      ),
    }))
